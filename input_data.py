...
class InputData:
    ...
    def add_data_sa(self, path_x, path_y, key='train'):
        self.x[key] = sa.attach(path_x)
        self.y[key] = sa.attach(path_y)
        print('data size x:', self.x[key].shape)
        print('data size y:', self.y[key].shape)
    ...
    def get_batch(self, idx=0, data_size=None, key='train'):
        ...
        ##return [self.x[key][st+8]* 2. - 1.]*data_size, [self.y[key][st+1]* 2. - 1.]*data_size ##
        
        return self.x[key][st:st+data_size] * 2. - 1., self.y[key][st:st+data_size] * 2. - 1.
