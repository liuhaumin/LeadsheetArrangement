...
class Metrics(object):
    ...
    def metric_polyphonic_ratio(self, bar, threshold=2):
        ...
        return sum(np.sum(bar, axis=1) >= threshold) / 48
    ...
    def metric_drum_pattern(self, bar):
        ...
        for i in range(6):
            ...
            ##print('cdf',np.shape(cdf),'temporal',np.shape(temporal))
    ...
    def metrics_harmonicity(self, chroma1, chroma2, resolution=12):
        score_list = []
    ...
    def eval(self, batch, output_type=0, quiet=False, save_fig=False, fig_dir='./'):
        ...
        batch =  np.reshape(batch,(-1, 48, 84, 5))
